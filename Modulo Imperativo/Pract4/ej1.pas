...
de unidades vendidas y el monto total. }
  ...
  var p: productoVendido;
     ...
     begin
       ...
       p.montoTotal:= v.cantUnidades * v.precioUnitario;  // calcula monto de venta
     end;
  ...
  Begin
    ...
    then begin
         ...
         then begin
                a^.dato.cantTotalUnidades:= a^.dato.cantTotalUnidades + elem.cantUnidades;  // agrego unidades vendidas al mismo producto
                a^.dato.montoTotal:= a^.dato.montoTotal + (elem.cantUnidades * elem.precioUnitario);  //agrego monto venta al total almacenado
              end
         else if (elem.codigoProducto < a^.dato.codigo) 
  ...
  begin
	if (a <> nil) then begin
		if (a^.dato.codigo < cod) then
			...
			CantidadDeCodigosMenores := CantidadDeCodigosMenores (a^.HI, cod);  // si es mayor igual al cod ingresado si o si va por izq 
	...
	if (a <> nil) then begin
		if (a^.dato.codigo > codigo1) and (a^.dato.codigo < codigo2) then 
			...
			if (a^.dato.codigo <= codigo1) then
				...
				ObtenerMontoTotalEntreDosCodigos := ObtenerMontoTotalEntreDosCodigos(a^.HI, codigo1, codigo2)  // no es necesario poner el if ya que es el ultimo caso que queda evaluar 
	end
	else
...
Begin
  ...
  ModuloE (a);   
End.
{correccion modulo entre, es lo mismo poner if .. else .. en vez de if else if 
* campos del reg con mismo nombre ? si se puede 
* }
